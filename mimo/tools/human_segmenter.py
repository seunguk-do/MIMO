    ...
    def run(self, img):
        ...
        output_img_value, logits_value = self.sess.run(
            [
                ...
                self.sess.graph.get_tensor_by_name("if_person:0"),
            ],
...
class hair_segmenter(object):
    ...
    def run_head(self, image):
        ...
        output_img_value = self.sess.run(
            self.sess.graph.get_tensor_by_name("output_alpha:0"),
            feed_dict={"input_image:0": image_feed},
        )
        # mask = output_img_value[:,:,-1]
    ...
    def pad_box(
        self, y1, y2, x1, x2, left_ratio, right_ratio, top_ratio, bottom_ratio, h, w
    ):
        box_w = x2 - x1
        box_h = y2 - y1
...
if __name__ == "__main__":
    ...
    fp = human_segmenter(model_path=f"{DATA_DIR}/assets/matting_human.pb")
