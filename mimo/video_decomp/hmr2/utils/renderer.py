...
vc_path = os.path.join(data_dir, "video_decomp/assets/sdc_info.npy")
vc_info = np.load(vc_path, allow_pickle=True)
print(vc_info)
vc_smpl = vc_info.item().get("smpl")
print("vc_smpl:", vc_smpl.shape)
...
class Renderer:
    ...
    def __call__(
        ...
        if full_frame:
            ...
            if self.cfg is None:
                ...
                img_std = self.cfg.MODEL.IMAGE_STD
                img_mean = self.cfg.MODEL.IMAGE_MEAN

            image = image.clone() * torch.tensor(img_std, device=image.device).reshape(
                3, 1, 1
            )
            image = image + torch.tensor(img_mean, device=image.device).reshape(3, 1, 1)
    ...
    def render_rgba(
        ...
        scene = pyrender.Scene(
            bg_color=[*scene_bg_color, 0.0], ambient_light=[1.0, 1.0, 1.0]
        )
        scene.add(mesh, "mesh")
    ...
    def render_rgba_multiple(
        ...
        if all_faces is not None:
            mesh_list = []
            for vvv, ttt, faces, color in zip(vertices, cam_t, all_faces, all_colors):
                tmp = pyrender.Mesh.from_trimesh(
            ...
            for vvv, ttt in zip(vertices, cam_t):
                print("vvv:", vvv)
                print("ttt:", ttt)
                trimesh_tmp = self.vertices_to_trimesh_ori(
                    vvv, ttt.copy(), mesh_base_color, rot_axis, rot_angle
                )
                tmp = pyrender.Mesh.from_trimesh(trimesh_tmp)
                mesh_list.append(tmp)
        ...
        #     # tmp.vertices
        #     # mesh = trimesh.Trimesh(vert, self.faces.copy(), vertex_colors=vc_smpl)
        #     mesh.export('tmp.obj')

        # vert = mesh_list[0].vertices
        ...
        # color, rend_depth = renderer.render(scene, flags=pyrender.RenderFlags.RGBA)
        # color, rend_depth = renderer.render(scene, flags=pyrender.constants.RenderFlags.FLAT)
        # color, rend_depth = renderer.render(scene, flags=pyrender.RenderFlags.FLAT)
        if hasattr(pyrender, "constants"):
